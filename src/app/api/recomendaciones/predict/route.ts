import { corsOptions, methodNotAllowed, readJsonBody } from "@/lib/api";
import { imputeClimateFromAltitude, predictCropsMl } from "@/lib/cropMl/cropPredictor";
import { json } from "@/lib/utils";

export async function OPTIONS() {
  return corsOptions();
}

export async function POST(request: Request) {
  const body = await readJsonBody<{
    lat?: number;
    lng?: number;
    altitud?: number;
    altitud_msnm?: number;
    mes?: number;
    temp_min_c?: number;
    temp_max_c?: number;
    precipitacion_mm_semana?: number;
  }>(request);

  const lat = Number(body.lat);
  const lng = Number(body.lng);
  if (Number.isNaN(lat) || Number.isNaN(lng)) {
    return json({ ok: false, error: "lat y lng son obligatorios" }, 400);
  }

  const altitud_msnm = Number(body.altitud_msnm ?? body.altitud ?? 3200);
  const mes = Number(body.mes ?? new Date().getMonth() + 1);
  const imp = imputeClimateFromAltitude(altitud_msnm);

  const temp_min_c = Number(body.temp_min_c ?? imp.temp_min_c);
  const temp_max_c = Number(body.temp_max_c ?? imp.temp_max_c);
  const precipitacion_mm_semana = Number(
    body.precipitacion_mm_semana ?? imp.precipitacion_mm_semana
  );

  const result = predictCropsMl({
    lat,
    lng,
    altitud_msnm,
    mes,
    temp_min_c,
    temp_max_c,
    precipitacion_mm_semana
  });

  return json({
    ok: true,
    ...result,
    ubicacion: { lat, lng, altitud_msnm },
    climaUsado: { temp_min_c, temp_max_c, precipitacion_mm_semana, mes }
  });
}

export async function GET() {
  return methodNotAllowed();
}
